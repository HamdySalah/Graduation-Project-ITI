...
export default function ActiveRequests() {
  ...
  useEffect(() => {
    // Check if user is a verified nurse before loading data
    if (user?.role === 'nurse' && user?.status === 'verified') {
      loadActiveRequests();
    } else if (user?.role === 'nurse' && user?.status !== 'verified') {
      // If nurse is not verified, don't load data and show proper message
      setLoading(false);
      setError('You must complete your profile and be verified by an administrator before accessing active requests.');
    }
  }, [user]);
  ...
  if (user.role === 'nurse' && user.status !== 'verified') {
    return (
      <Layout>
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900 mb-4">Profile Verification Required</h1>
            <p className="text-gray-600 mb-4">You must complete your profile and be verified by an administrator before accessing active requests.</p>
